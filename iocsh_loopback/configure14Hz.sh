...
if hash ${CAPUT} 2>/dev/null ; then
    ...
    ${CAPUT} -a ${PREFIX}:SoftSeq-0-EvtCode-SP 2 ${EventCode} ${EndOfSeqEvent} > /dev/null
    # Defining time at which the event codes are sent in us
    ${CAPUT} -a ${PREFIX}:SoftSeq-0-Timestamp-SP 2 0 1 > /dev/null

    # Commit the sequence to HW
    ${CAPUT} ${PREFIX}:SoftSeq-0-Commit-Cmd 1 > /dev/null
else
    printf "\n>>>> We cannot run $0\n";
