...
main()
{
  ...
  //CLOCK_MONOTONIC_RAW
  if (timespec2str(timestr, sizeof(timestr), &raw) != 0) {
    printf("timespec2str failed!\n");
    return EXIT_FAILURE;
  } else {
    unsigned long resol = raw.tv_sec * NANO + raw.tv_nsec;
    printf("CLOCK_MONOTONIC_RAW: %s (raw=%ld ns)\n", timestr,resol);
  }
