...
epicsEnvSet("IOC", "MCAG:TS")
epicsEnvSet("SYS",          "$(IOC)")
epicsEnvSet("DEV",          "EVR-01")
epicsEnvSet("EPICS_CMDS",   "/opt/iocs")
epicsEnvSet("MRF_HW_DB",    "evr-mtca-300-ess.db")
epicsEnvSet("PCI_SLOT",     "08:0.0")
...
# EVR PCI setup
# -----------------------------------------------------------------------------
iocshLoad("$(mrfioc2_DIR)/evr-mtca-init.iocsh", "S=$(IOC)-, DEV=$(DEV), PCIID=$(PCIID)")
...
iocshLoad("$(mrfioc2_DIR)/evr-standalone-mode.iocsh", "IOC=$(IOC)-, DEV=$(DEV)")
